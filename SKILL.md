...
整个 skill 目录（SKILL.md + scripts/）可整体拷贝独立部署。
...
机械检查项（字面量保真 + 版本漂移 + 证据错误含文件名重复）由 `wiki_check.py` 执行；Status 块格式与断链/孤儿优先用 obsidian-mcp-pro（`find_broken_links` / `get_backlinks` / `find_orphans`），无 MCP 环境时全文检索兜底。

**报告的嫌疑人/漂移不是判决**——派生值、产品名、页面录入日期会误报；AI 只做语义复核与处置。修复权限两档：

- **自动修**：断链按文件名检索，恰好一个匹配 → 修；零个或多个 → 报告
- **只报告**：字面量失配、版本漂移、malformed Status 块、孤儿页——事实类问题永不自动修

Lint 结束后在 `log.md` 的 `## lint` 小节追加一行结论。
...
- 链接固定 Obsidian 双链 `[[页面名]]`（vault 根 = project-root），与目标文件名完全一致，**不含目录路径**；文件名全 vault 唯一（同名即歧义）。双链解析与目录无关，迁移子目录零链接成本。
- 建页前先查检索面：近似概念**更新而非新建**；同一概念禁止两个页面。

### 检索面与 index（本地投影缓存）

- 任何操作第一步经过同一可复现的检索面：优先 MCP（`search_notes` / `list_tags`），无 MCP 时文件枚举（文件名即主题索引）。
...
每个页面必须声明证据坐标，无来源的论断不得写入（AI 推断须标注"推断"+依据）。来源页 frontmatter 字段（完整模板见文末）：

| 字段 | 语义 |
|------|------|
| `raw` | 证据路径：本仓库相对路径，禁止绝对盘符/外部仓库。跨仓库材料先拷副本入 `docs/`（命名带版本号，升级另存不覆盖）；同仓库直接引用；同一逻辑来源（共享版本号）多文件写数组 |
| `version` | 语义锚点：录入时源文档的版本号；无版本号省略（验证降级） |
| `date` | 源文档日期；录入时间由 log 承载 |

- 字段准入：仅允许 `tags / aliases / raw / version / date`；`type`、`name` 等与文件名重复的字段禁止。
- 来源粒度：一个 src 页 = 一个逻辑来源（判据：是否共享版本号）；独立文档各建一页，多源关系由 con 页来源区表达；文档升级不新建 src，走 Update 刷新 `raw`/`version`。
- locate-before-write：每个数字/版本/日期/引语写入前先在源文定位；源文写 42K 就写 42K；找不到出处就不写精确值；推导值展示分量。
- 概念页以「来源」区 `[[src-xxx]]` 回指，坐标统一由 src 页持有。
- dec- 页例外：`raw` 可选（决策依据文档，有则写）；`date` = 提出/决策日期；无文档依据时在「依据」区写明决策人与场合。
...
旧论断不删，禁止静默改写被推翻的论断。格式（紧贴受影响论断正下方）：
...
- Outdated：日期 + 变更说明 + 来源，缺一即 malformed（lint 检查）。
- Disputed：来源打架时两边页面都标，并列写各自出处，不裁决。
- dec- 页的关闭：**Resolved**（已解决，附结论）/ **Superseded**（被 `[[dec-xxx]]` 取代）——同样带日期，lint 同查三要素。
- 漂移闭环：lint 发现 `version` 落后 → 复核 → 过时论断加 Status 块、有效论断改写并刷新锚点。

### 操作日志（log.md）

只追加，永不修改或删除。Ingest 全部 disposition（含 No material）与 Lint 结论记入；Query 不记录。**访问纪律：只允许追加（所属小节末尾）/ 读尾部 / grep 定点检索，禁止全文读入上下文。**

格式（按来源分小节，小节按来源名排序插入——并发写入落不同区域，git 自动合并）：
...
冲突兜底：两边条目全保留，按时间戳排序。初始化：首次 Ingest 创建 `# Wiki Log` 标题，之后只追加。
...
- **小节化写入纪律**：每次更新只动所属小节（同 log 分小节的冲突预防原理——不同文档流触发的更新落在不同小节，git 自动合并）。
- **膨胀拆分**：按**子主题**拆 `syn-<子主题>`（各自继承多维结构），本页降级为索引——**禁止按文档类型拆回三页**。
- 知识维度不限于需求/设计/调试，按主题实际涉及的来源类型定。

## 脚本工具（scripts/）

四个确定性脚本，零第三方依赖（Python 3.8+ 标准库），报告即接口（退出码不承载信息）；除 `rebuild_index.py` 写 `wiki/index.md` 外一律 report-only：
...
脚本报告的嫌疑人/漂移**不是判决**——语义复核与处置归 AI。

### 需求侧度量（§10）操作规程

需求侧度量依赖 obsidian-mcp-pro 在**工具调用层零 LLM 采集**真实查询行为（§10.3 前提）。两端分工：

- **obsidian-mcp-pro 端（采集）**：`src/lib/monitor.ts` 经 `OBSIDIAN_TELEMETRY` 开关发射原始调用事件（每次工具调用一行 JSON：工具名/生效 vault/路径或 query/是否出错/结果大小/耗时），只发射不聚合、不引入 wiki 结构知识。`tool-seam.ts` 的 `defineTool` 是唯一 hook 点，一处覆盖全部 41 工具。
- **本 skill 端（聚合）**：`wiki_metrics.py` 把事件流做查询归并（§10.4，零 LLM 三层启发式）→ 算维度 4–8（§10.3）→ 按 `raw` 坐标聚合到每个源文档（§10.5）→ 可选 `--baseline`/`--report compare` 闭环对比（§10.6）。
...
诚实边界（§10.7）：仅记录走 obsidian-mcp-pro 的调用；UI/manual 查询采集不到须标注。检索未命中用"短搜索回文"代理（可调 `--miss-bytes`），属近似。报告只给行为统计与 refine 优先级队列，不产出"质量分数"。度量只观测、不参与 Ingest/Lint 主流程；refine 优先级清单作为**行动建议**交 AI 复核。
